...
@section('title',' ' .$url. ' | News | Archived')
        ...
        <div class="admin-wrap-article">
          ...
          @foreach ($id as $news)
            <div class="table_row row">
              ...
              <div class="table_img">
                <a href="{{ url('news/upload/'. $news->id . '-' . str_replace(' ', '-', $news->title)) }}">
              ...
              <div class="dropdown2">
                ...
                <div id="m5" class="dropdown2-content">
                  ...
                  <div class="index-btn-row">
                    <a href="{{ url($url.'/editors/'.$news->editor_id) }}">
                    <i class="fa fa-user-cog"></i> Editor</a>
                  </div>

                  <div class="index-btn-row">
                    <a href="{{ url($url.'/news/preview/'. $news->id . '-' . str_replace(' ', '-', $news->title)) }}">
                      <i class="fa fa-eye"></i> Preview
                    </a>
                  </div>

                  <div class="index-btn-row">
                    <a href="{{ url($url.'/news/dashboard/'. $news->id . '-' . str_replace(' ', '-', $news->title)) }}">
                  ...
                  <div class="index-btn-row">
                    <span class="row">
                      <i class="fa fa-calendar-check"></i> Published {{ date('d M Y @ h:i', strtotime($news->publish)) }}
                    </span>
                  </div>
                  <div class="index-btn-row">
                    <span class="row">
                      <i class="fa fa-calendar-times"></i> Archived {{ date('d M Y @ h:i', strtotime($news->archive)) }}
                    </span>
                  </div>
